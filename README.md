...
Modern teams make decisions everywhere:

- GitHub has code activity and pull requests.
- Slack has discussions, blockers, and informal decisions.
- Linear and Jira have delivery status.
- Confluence and Notion have docs and decision records.
- Zendesk has customer pain and escalation signals.
- Meetings contain action items and context that often disappears.

The result is fragmented knowledge. People waste time asking:

- What happened with this feature?
- Why did we make this decision?
- Is this Jira issue duplicated in Linear?
- Which customer signals mention this engineering work?
- What action items came from the last meeting?
- What evidence supports this answer?

ClosedLoop OS solves this by creating a unified event intelligence layer across tools.
...
All connectors normalize into the same event shape:
...
- creates `local.settings.json` from `local.settings.sample.json` if needed
...
- `query_github_events(project=None, actor=None, event_type=None, limit=25)`
- `get_event_by_id(event_id)`
- `search_decisions(query_text=None, limit=25)`
- `get_slack_context(channel, thread_ts=None, user=None, limit=25)`
- `get_linear_sprint_status(project=None, cycle=None, limit=25)`
- `semantic_search(query_text, limit=10, source_tool=None)`
- `get_jira_epic_status(epic_key=None, project=None, limit=25)`
- `get_notion_decisions(query_text=None, limit=25)`
- `analyze_meeting(meeting_id, limit=100)`
- `get_customer_signals(limit=25)`
...
Once events are ingested, the intelligence layer can answer questions like:
...
Which Jira and Linear issues look duplicated?
```

`ask_intelligence(question)` returns:
...
The implementation is intentionally conservative: factual claims are built from retrieved internal sources and should include citations.
